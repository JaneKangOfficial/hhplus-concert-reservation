...
public class QueuesServiceImpl implements QueuesService {
    ...
    // QueueRepository에서 QueuesRedisRepositoryImpl를 호출해야 한다!
    public QueuesServiceImpl(QueueRepository queueRepository) {
    ...
    public Long entryWaitingQueue(Long userId) {
        ...
        if (rank == null) {
            ...
            queueRepository.addQueue(userId, WAIT_QUEUES);    // 없다면 대기열 진입 (있는데 또 넣으면 새로고침)
            queueRepository.expireQueue(WAIT_QUEUES, 60 * 30);  // TTL 설정 30분
            rank = queueRepository.getRank(userId, WAIT_QUEUES);
        }
    ...
    public void enqueueActiveUsers() {
        List<Tuple> rangeList = queueRepository.getRangeWithScores(WAIT_QUEUES, 0L, 49L); // 50명씩 가져오기

        rangeList.stream().forEach(r -> {
            queueRepository.delQueue(Long.valueOf(r.getElement()), WAIT_QUEUES); // 대기열 삭제
            queueRepository.addQueue(Long.valueOf(r.getElement()), ACTIVE_QUEUES + ":" + r.getElement()); // 활성으로 진입 / keys ACTIVE_QUEUES:* 명령어 치면 풀스캔 (* 운영에서 하면 안된다! 레디스는 싱글 스레드이다!)
            queueRepository.expireQueue(ACTIVE_QUEUES + ":" + r.getElement(), 60 * 5);  // TTL 설정 5분
        });
    }
...
/*    @Override
    public void expiredQueue() {
        List<Tuple> rangeList = queueRepository.getRangeWithScores(ACTIVE_QUEUES, 0L, 49L); // 50명씩 가져오기
        ...
        rangeList.stream().forEach(s -> {
            ...
            if (currentTimeMillis - score > 1800000) {    // 30분
                ...
                queueRepository.delQueue(Long.valueOf(s.getElement()), ACTIVE_QUEUES);
